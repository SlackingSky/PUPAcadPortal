...
namespace PUPAcadPortal
{
    public partial class SubmissionList : UserControl
    {
        ...
        {
            var checked_ = submissions.FindAll(s => s.IsChecked);

            if (checked_.Count == 0)
            {
                MessageBox.Show(
                    "No checked submissions to return.",
                    "Return All",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
                return;
            ...
            if (confirm == DialogResult.Yes)
            {
                ...
                MessageBox.Show(
                    $"{checked_.Count} submission(s) returned successfully.",
                    "Return All",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
