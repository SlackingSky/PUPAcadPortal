...
namespace PUPAcadPortal.PortalContents.Student.LMS.Course
{
    public partial class StudentActivityList : UserControl
    {
        ...
        public StudentActivityList(StudentCourse course)
            : this(course, 0, new NullStudentCourseDbService())
        {

        }

        /// <summary>Full DB-backed constructor used at runtime.</summary>
        ...
        {
            if (_studentId <= 0 || string.IsNullOrWhiteSpace(_course.SubjectOfferingId))
            {
        ...
        private void RenderRows()
        {
            ...
            {
                if (filter != "All" &&
                    !act.Type.Equals(filter, StringComparison.OrdinalIgnoreCase)) continue;

                if (status != "All Status" &&
                    !act.EffectiveStatus.Equals(status, StringComparison.OrdinalIgnoreCase)) continue;
            ...
            if (!anyVisible)
            {
                string msg = _activities.Count == 0
                    ? "No activities have been published for this course yet."
        ...
        private Panel BuildRow(StudentActivityItem act)
        {
            ...
            row.Controls.Add(new Label
            {
                Text = ptsText,
                Font = new Font("Segoe UI", 8.5F,
                                act.Score.HasValue ? FontStyle.Bold : FontStyle.Regular),
                ForeColor = act.Score.HasValue
            ...
            // Action button
            bool locked = act.LockAfterDeadline
                          && act.Deadline < DateTime.Now
                          && act.SubmissionStatus == "Pending";

            string btnLbl = locked ? "Locked \uD83D\uDD12" :
                            isReturned ? "View Remarks" :
            ...
            // Secondary "View Remarks" link for returned items
            if (isReturned)
            {
        ...
        private void ShowRemarksDialog(StudentActivityItem act)
        {
            ...
            var pnlH = new Panel { Dock = DockStyle.Top, Height = 52, BackColor = Color.Maroon };
            ...
            var body = new Panel { Dock = DockStyle.Fill, AutoScroll = true, Padding = new Padding(20) };
            int y = 16;
            int bodyW = 530;

            if (!string.IsNullOrEmpty(act.ScoreBreakdown))
            {
                body.Controls.Add(new Label { Text = "Score Breakdown:", Font = new Font("Segoe UI", 9.5F, FontStyle.Bold), ForeColor = Color.FromArgb(30, 30, 30), Location = new Point(16, y), AutoSize = true }); y += 22;
                body.Controls.Add(new Label { Text = act.ScoreBreakdown, Font = new Font("Segoe UI", 9F), ForeColor = Color.FromArgb(70, 70, 70), Location = new Point(16, y), Size = new Size(bodyW, 18) }); y += 30;
            }

            body.Controls.Add(new Label { Text = "Instructor Feedback:", Font = new Font("Segoe UI", 9.5F, FontStyle.Bold), ForeColor = Color.FromArgb(30, 50, 160), Location = new Point(16, y), AutoSize = true }); y += 22;

            int feedbackH = Math.Max(70, MeasureTextHeight(act.Remarks, new Font("Segoe UI", 9.5F), bodyW - 24) + 24);
            var pnlFeed = new Panel { BackColor = Color.FromArgb(232, 238, 255), Location = new Point(16, y), Size = new Size(bodyW, feedbackH), Padding = new Padding(12) };
            pnlFeed.Paint += (s, e) => { using var pen = new Pen(Color.FromArgb(190, 205, 240)); e.Graphics.DrawRectangle(pen, 0, 0, pnlFeed.Width - 1, pnlFeed.Height - 1); };
            pnlFeed.Controls.Add(new Label { Text = act.Remarks, Font = new Font("Segoe UI", 9.5F), ForeColor = Color.FromArgb(30, 30, 80), Location = new Point(12, 10), Size = new Size(bodyW - 28, feedbackH - 20) });
            body.Controls.Add(pnlFeed);
            y += feedbackH + 16;

            if (!string.IsNullOrEmpty(act.Suggestions))
            {
                body.Controls.Add(new Label { Text = "Suggestions for Improvement:", Font = new Font("Segoe UI", 9.5F, FontStyle.Bold), ForeColor = Color.FromArgb(50, 50, 50), Location = new Point(16, y), AutoSize = true }); y += 22;
                int suggH = Math.Max(60, MeasureTextHeight(act.Suggestions, new Font("Segoe UI", 9F), bodyW - 24) + 24);
                var pnlSugg = new Panel { BackColor = Color.FromArgb(255, 248, 220), Location = new Point(16, y), Size = new Size(bodyW, suggH), Padding = new Padding(12) };
                pnlSugg.Paint += (s, e) => { using var pen = new Pen(Color.FromArgb(240, 210, 140)); e.Graphics.DrawRectangle(pen, 0, 0, pnlSugg.Width - 1, pnlSugg.Height - 1); };
                pnlSugg.Controls.Add(new Label { Text = act.Suggestions, Font = new Font("Segoe UI", 9F), ForeColor = Color.FromArgb(100, 70, 0), Location = new Point(12, 10), Size = new Size(bodyW - 28, suggH - 20) });
            ...
            if (act.ReturnedAt.HasValue)
            {
                body.Controls.Add(new Label { Text = $"Returned on  {act.ReturnedAt:dddd, MMMM dd, yyyy  h:mm tt}", Font = new Font("Segoe UI", 8F, FontStyle.Italic), ForeColor = Color.Gray, Location = new Point(16, y), AutoSize = true }); y += 28;
            }

            var btnClose = new buttonRounded { Text = "Close", Size = new Size(100, 36), BackColor = Color.Maroon, ForeColor = Color.White, BorderRadius = 18, Font = new Font("Segoe UI", 9.5F, FontStyle.Bold), FlatStyle = FlatStyle.Flat, Location = new Point(bodyW - 84, y + 4), DialogResult = DialogResult.OK };
